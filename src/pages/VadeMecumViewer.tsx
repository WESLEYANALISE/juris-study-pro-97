...
const VadeMecumViewer = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from(tableName).select("*").order('id');
      if (error) {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('vademecum_history').select('*').order('viewed_at', {
        ascending: false
      }).limit(5);
      if (error) {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('vademecum_favorites').select('*').order('created_at', {
        ascending: false
      });
      if (error) {
  ...
  return <div className="container mx-auto p-6 px-[14px]">
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <Input placeholder="Buscar artigos..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10" />
            </div>
            <DropdownMenu>
          ...
          <div className="space-y-6">
            ...
            length: 3
          }).map((_, i) => <Skeleton key={i} className="h-32" />) : filteredArticles.length === 0 ? <div className="text-center py-12">
                <p className="text-muted-foreground">
                  Nenhum artigo encontrado com os critérios de busca.
                </p>
              </div> : filteredArticles.map(article => <ArticleCard key={article.id} lawName={tableName || ''} articleNumber={article.numero || ''} articleText={article.artigo} technicalExplanation={article.tecnica} formalExplanation={article.formal} practicalExample={article.exemplo} fontSize={fontSize} onFontSizeChange={setFontSize} />)}
          </div>
        </div>
          ...
          <Tabs defaultValue="favorites" className="w-full">
            ...
            <TabsContent value="favorites">
              <div className="space-y-2">
                {favorites.map(favorite => <Button key={favorite.id} variant="ghost" className="w-full justify-start text-left" onClick={() => navigate(`/vademecum/${favorite.law_name}`)}>
            ...
            <TabsContent value="history">
              <div className="space-y-2">
                {recentHistory.map(item => <Button key={item.id} variant="ghost" className="w-full justify-start text-left" onClick={() => navigate(`/vademecum/${item.law_name}`)}>
                    <div className="truncate">
      ...
      <Button className="fixed bottom-6 right-6 z-10" size="icon" onClick={scrollToTop}>
