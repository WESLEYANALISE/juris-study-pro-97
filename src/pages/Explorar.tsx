...
const Explorar = () => {
  ...
  return <div className="container mx-auto py-6 max-w-5xl px-0">
      ...
      <div className="space-y-8">
        {allFeatures.map((category, categoryIndex) => <div key={categoryIndex}>
            <h2 className="text-xl font-semibold mb-4 px-2">
              {category.category}
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {category.items.map((feature, index) => <Card key={index} className="overflow-hidden shadow-sm hover:shadow-md transition-shadow border h-full">
                  ...
                  <CardFooter className="p-3 pt-0">
                    <Button variant="outline" onClick={() => navigate(feature.path)} className="w-full text-xs min-h-[48px]">
                      Acessar
                    </Button>
                  </CardFooter>
                </Card>)}
