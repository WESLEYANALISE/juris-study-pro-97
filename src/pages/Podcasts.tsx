...
const Podcasts = () => {
  ...
  return <JuridicalBackground variant="scales" opacity={0.03}>
      <div className="container mx-auto px-2 md:px-4 py-6 pb-24">
        ...
        duration: 0.5
          ...
          <div className="relative inline-block mx-auto">
            ...
            <motion.span className="absolute -inset-8 rounded-full opacity-10 bg-primary blur-xl" initial={{
            scale: 0.8,
            opacity: 0
          }} animate={{
            scale: 1,
        ...
        {selectedPodcast && <div className="mb-6">
            {isMobile ? <AudioPlayerMobile id={selectedPodcast.id} title={selectedPodcast.title} audioUrl={selectedPodcast.audio_url} thumbnail={selectedPodcast.thumbnail_url} onError={handleAudioError} onEnded={() => {
          toast.success("Podcast concluído!");
        }} /> : <AudioPlayer id={selectedPodcast.id} title={selectedPodcast.title} audioUrl={selectedPodcast.audio_url} thumbnail={selectedPodcast.thumbnail_url} duration={selectedPodcast.duration} />}
            
            {audioError && <Alert variant="destructive" className="mt-3">
            ...
            {!audioError && <motion.div className="mt-4 bg-primary/5 p-4 rounded-lg border border-primary/20" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          delay: 0.2
                ...
                {selectedPodcast.categories && selectedPodcast.categories.length > 0 && <div className="flex flex-wrap gap-2 mt-3">
                    <span className="text-xs text-muted-foreground">Categorias:</span>
                    {selectedPodcast.categories.map(category => <span key={category.slug} className="text-xs text-primary bg-primary/10 px-2 py-0.5 rounded cursor-pointer" onClick={() => {
              ...
              setSelectedPodcast(null);
                ...
                {selectedPodcast.tags && selectedPodcast.tags.length > 0 && <div className="flex flex-wrap gap-2 mt-2">
                    <span className="text-xs text-muted-foreground">Tags:</span>
                    {selectedPodcast.tags.map((tag, index) => <span key={index} className="text-xs text-muted-foreground">#{tag}</span>)}
                  </div>}
              </motion.div>}
          </div>}

        {/* Recently played podcasts - only show if no podcast is currently selected */}
        {!selectedPodcast && user && <RecentPodcasts onSelectPodcast={handleSelectPodcast} limit={isMobile ? 2 : 4} />}

        {/* Featured Podcasts */}
        {featuredPodcasts.length > 0 && !selectedPodcast && <motion.div className="mb-6" initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        delay: 0.2
            ...
            {isMobile ? <div className="grid grid-cols-2 gap-3">
                {featuredPodcasts.slice(0, 2).map(podcast => <Card key={podcast.id} className="cursor-pointer hover:shadow-md transition-shadow overflow-hidden border-primary/10" onClick={() => handleSelectPodcast(podcast)}>
                    <div className="relative aspect-video overflow-hidden">
                      <img src={podcast.thumbnail_url || "https://via.placeholder.com/300x200?text=Podcast"} alt={podcast.title} className="w-full h-full object-cover" />
                ...
                <CarouselContent>
                  {featuredPodcasts.map(podcast => <CarouselItem key={podcast.id} className="md:basis-1/2 lg:basis-1/3">
        ...
        <PodcastFilters onSearchChange={setSearchTerm} onCategoryChange={setSelectedCategory} onSortChange={setSortBy} categories={categories} selectedCategory={selectedCategory} />
        ...
        <Tabs defaultValue="all" value={currentTab} onValueChange={setCurrentTab}>
          ...
          <TabsContent value="all">
            <PodcastList searchTerm={searchTerm} category={selectedCategory} sortBy={sortBy} onSelectPodcast={handleSelectPodcast} />
          </TabsContent>
          
          <TabsContent value="favorites">
            <PodcastList searchTerm={searchTerm} category={selectedCategory} sortBy={sortBy} showFavoritesOnly={true} onSelectPodcast={handleSelectPodcast} />
          </TabsContent>
          
          <TabsContent value="inProgress">
            {inProgressPodcasts.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {inProgressPodcasts.map(podcast => <motion.div key={podcast.id} initial={{
              opacity: 0,
              y: 20
            }} animate={{
              opacity: 1,
              ...
              duration: 0.3
                    ...
                    <PodcastCard id={podcast.id} title={podcast.title} description={podcast.description} thumbnail={podcast.thumbnail_url} duration={podcast.duration} publishedAt={podcast.published_at} progress={podcast.progress} categories={podcast.categories} tags={podcast.tags} area={podcast.area} onClick={() => handleSelectPodcast(podcast)} />
                  </motion.div>)}
