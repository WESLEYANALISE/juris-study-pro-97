...
const categories = ["Todos", "Legislação", "Concursos", "Jurisprudência", "Tecnologia", "Administrativo", "Notícias"];
const sources = ["Todos", "Jus Brasil", "Consultor Jurídico", "Migalhas", "Conjur", "Estratégia Concursos"];
const Bloger = () => {
  ...
  return <div className="container mx-auto px-[10px]">
      <div className="mb-6">
      ...
      <div className="mb-6">
        <div className="relative">
          ...
          <Input className="pl-10" placeholder="Buscar artigos, temas ou autores..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
      </div>
      ...
      <Tabs defaultValue="categorias" className="mb-8">
        ...
        <TabsContent value="categorias" className="space-y-4">
          <div className="flex flex-wrap gap-2">
            {categories.map(category => <Button key={category} variant={activeCategory === category ? "default" : "outline"} size="sm" onClick={() => setActiveCategory(category)}>
                {category}
              </Button>)}
          </div>
        </TabsContent>
        ...
        <TabsContent value="fontes" className="space-y-4">
          <div className="flex flex-wrap gap-2">
            {sources.map(source => <Button key={source} variant="outline" size="sm">
                {source}
              </Button>)}
          </div>
        </TabsContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-0 my-0 mx-0 px-0">
        {filteredArticles.map(article => <Card key={article.id} className="overflow-hidden">
          ...
          backgroundImage: `url(${article.imageUrl})`
            ...
            <CardHeader>
              <div className="flex justify-between items-start">
                ...
                <Button variant="ghost" size="icon" onClick={() => narrarTexto(`${article.title}. ${article.summary}`)}>
