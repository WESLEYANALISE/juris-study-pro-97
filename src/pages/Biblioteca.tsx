...
const Biblioteca = () => {
    ...
    const fetchBooks = async () => {
      try {
        ...
        if (savedRecentBooks) {
          ...
          const recentBooksData = loadedBooks.filter(book => 
            recentIds.includes(book.id)
          ).slice(0, 5);
  ...
  const addToRecentBooks = (book: Book) => {
    ...
    const updatedRecentBooks = recentIds.map(id => 
    ...
    return (
      ...
      >
        <div className="relative w-[150px] h-[220px] rounded-md overflow-hidden shadow-lg group">
          <div 
            className="w-full h-full bg-gradient-to-b from-primary/20 to-primary/40 flex items-center justify-center text-center p-2"
            style={{
