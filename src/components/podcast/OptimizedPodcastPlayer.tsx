...
interface OptimizedPodcastPlayerProps {
  id: string;
  title: string;
  description: string;
  audioUrl: string;
...
const Comment = React.memo(({ 
  ...
  const handleJumpClick = useCallback(() => {
    onJumpToTime(comment.timestamp);
  }, [comment.timestamp, onJumpToTime]);
  ...
  return (
    <div className="border border-white/10 rounded-lg p-3">
      <div className="flex items-center justify-between mb-2">
        ...
        <Button variant="ghost" size="sm" onClick={handleJumpClick} className="space-x-1">
...
// Main optimized podcast player with performance improvements
export function OptimizedPodcastPlayer({
  id,
  title,
  description,
  audioUrl,
  ...
  // Only run this once to avoid recreating comments on every render
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Don't use timeupdate event, it fires too frequently and causes performance issues
    // Instead, use a custom interval that runs less frequently
  ...
  // Play/pause logic - optimized with fewer operations
  useEffect(() => {
    const audio = audioRef.current;
    if (!audio) return;
    ...
    if (isPlaying) {
      ...
      // Update progress less frequently (every 250ms instead of 100ms)
      if (!progressIntervalRef.current) {
  ...
  }, [commentText, currentTime, user]);
  
  const jumpToCommentTime = useCallback((time: number) => {
    const audio = audioRef.current;
    if (!audio) return;
  ...
  // Volume control - batched into a single effect
  useEffect(() => {
    const audio = audioRef.current;
    if (!audio) return;
  ...
  const visualizationColor = useMemo(() => {
  ...
  return (
    <motion.div 
      className="fixed inset-0 z-50 flex flex-col bg-gradient-to-b from-purple-950 to-black overflow-hidden"
      {...animations.fadeIn}
    >
      ...
      >
        ...
        <Button variant="ghost" size="icon" onClick={() => setIsFullView(!isFullView)}>
      ...
      >
        <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full">
          ...
          <TabsContent value="visualizer" className="p-4 h-full flex items-center justify-center overflow-hidden py-[17px] px-[15px] my-0 mx-[16px]">
            <div className="w-full max-w-lg">
              ...
              <div className="relative mx-auto rounded-lg overflow-hidden aspect-square max-w-xs mb-6">
                <img 
                  src={thumbnail || "/placeholder-podcast.jpg"} 
                  alt={title} 
                  ...
                  <AnimatePresence mode="wait">
                    {isPlaying ? (
                      ...
                      <motion.div 
                        ...
                        className="rounded-full bg-purple-600/80 p-4 hover:bg-purple-500/80 cursor-pointer" 
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-1">
                  <Button 
                    ...
                    size="sm" 
                    className={isLiked ? "bg-purple-600" : ""}
                    onClick={toggleLike}
                  >
                  ...
                  <Button 
                    ...
                    size="sm" 
                    className={isBookmarked ? "bg-purple-600" : ""}
          ...
          {/* Info tab - content loaded on demand to save memory */}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {categories.map((category, index) => (
                        <Badge key={index} variant="outline" className="bg-purple-900/40">
                          {category.name}
                        </Badge>
                      ))}
                    </div>
          ...
          {/* Comments tab - content loaded on demand to save memory */}
            ...
            {activeTab === 'comments' && (
              <div className="max-w-lg mx-auto">
                {user ? (
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        <textarea 
                          ...
                          placeholder="Compartilhe seus pensamentos sobre este momento do podcast..." 
                          className="w-full p-2 rounded-md text-sm bg-white/5 border border-white/10" 
                          rows={3} 
                        />
                  ...
                  <div className="mb-6 p-4 text-center border border-white/10 rounded-lg">
                ...
                <div className="space-y-4">
                  ...
                  {/* Use virtualized list for better performance with many comments */}
                  <div className="space-y-3">
                    {comments.map(comment => (
                      <Comment key={comment.id} comment={comment} onJumpToTime={jumpToCommentTime} />
                    ))}
                  </div>
