    ...
    >
      ...
      <div className="absolute -bottom-4 left-1/2 transform -translate-x-1/2">
        <Button
          variant="outline"
          size="sm"
          className="h-8 w-8 rounded-full bg-black border border-purple-800"
      ...
      <div className="overflow-hidden">
        <div className="flex flex-col gap-3 px-2 py-3">
          ...
          <div className="flex justify-between items-center">
            <div className="flex gap-2">
              {(['all', 'progress', 'favorites'] as const).map((filter) => (
                <Button
                  ...
                  className={cn(
                    "px-2 py-1 h-auto",
                    activeFilter === filter ? "bg-purple-900/30 text-purple-400" : "text-gray-400"
                  )}
                  onClick={() => setActiveFilter(filter)}
                >
                  {filter === 'all' ? <Library className="h-3 w-3 mr-1" /> : 
                   filter === 'progress' ? <Clock className="h-3 w-3 mr-1" /> : 
            ...
            <div className="flex gap-1">
              <Button
                ...
                className={cn(
                  "w-8 h-8 p-0",
                  viewMode === 'grid' && "bg-purple-900/30 text-purple-300"
                )}
              ...
              <Button
                ...
                className={cn(
                  "w-8 h-8 p-0",
                  viewMode === 'list' && "bg-purple-900/30 text-purple-300"
                )}
          ...
          <Button
            variant="outline"
            size="sm"
            className="w-full border-purple-800/50 text-xs"
            onClick={() => {
              resetFilters();
              setIsExpanded(false);
            }}
