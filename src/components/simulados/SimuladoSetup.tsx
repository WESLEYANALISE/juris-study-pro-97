        ...
        onStart({
          ...
          <div className="space-y-2">
            ...
            {loading ? <div className="flex justify-center p-4">
                <LoadingSpinner />
              </div> : <Select value={edicaoId} onValueChange={setEdicaoId} disabled={edicoes.length === 0}>
                <SelectTrigger className="w-full">
                  <SelectValue placeholder={edicoes.length === 0 ? "Nenhuma edição disponível" : "Selecione a edição"} />
                </SelectTrigger>
                <SelectContent>
                  {edicoes.map(edicao => <SelectItem key={edicao.id} value={edicao.id}>
          ...
          {selectedEdicao && <div className="bg-muted p-3 rounded-md text-sm">
              <p>Total de questões: {selectedEdicao.total_questoes}</p>
              <p>Ano: {selectedEdicao.ano}</p>
              <p>Número: {selectedEdicao.numero}</p>
            </div>}

          <Button type="submit" className="w-full" disabled={!edicaoId || loading}>
            Iniciar Simulado
          </Button>
        </form>
      </CardContent>
