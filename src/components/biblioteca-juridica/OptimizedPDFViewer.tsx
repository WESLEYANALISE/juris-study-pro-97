    ...
    >
      ...
      >
        {isLoading ? (
          ...
          >
            {numPages && pagesToRender.map(pageNum => (
              <div 
                id={`page-${pageNum}`}
                key={`page-${pageNum}`} 
              ...
              >
                <Page
                  ...
                  width={window.innerWidth > 1200 ? undefined : Math.min(window.innerWidth - 32, 800)}
                />
      ...
      <AnimatePresence>
        {controlsVisible && (
          <>
            ...
            {/* Bottom controls */}
            <motion.div
              className="w-full bg-black/80 backdrop-blur-md border-t border-white/10 flex flex-col"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 20 }}
              transition={{ duration: 0.2 }}
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div className="flex items-center space-x-1">
                    <Input
                      ...
                      className="w-12 h-8 text-center bg-white/10 border-white/20 text-white"
                    />
...
export const LazyOptimizedPDFViewer = React.lazy(() => 
  import('./OptimizedPDFViewer').then(module => ({ default: module.OptimizedPDFViewer }))
);
