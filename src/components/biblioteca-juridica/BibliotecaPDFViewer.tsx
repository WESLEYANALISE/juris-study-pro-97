...
import { 
  ChevronLeft, ChevronRight, X, Search, ZoomIn, ZoomOut, 
  Bookmark, BookmarkPlus, BookmarkMinus, Layers, Download,
  MessageSquare, Edit, Settings
} from 'lucide-react';
...
export function BibliotecaPDFViewer({
    ...
    try {
      ...
      if (progressData) {
        ...
        toast({
          ...
          description: progressData.favorito 
  ...
  return (
    <div className="fixed inset-0 bg-background/90 backdrop-blur-sm z-50 flex flex-col">
      <div className="bg-card shadow-lg p-4 border-b flex justify-between items-center transition-opacity duration-300"
      ...
      <div className="flex items-center justify-between p-2 bg-card/50 border-b transition-opacity duration-300"
          style={{ opacity: showControls ? 1 : 0 }}
      >
        ...
        <div className="flex items-center space-x-2">
          ...
          >
            {bookmarks.some(b => b.pagina === pageNumber) ? (
      ...
      <div ref={containerRef} className="flex-grow overflow-auto p-4 flex justify-center px-0">
        ...
        <div className="absolute top-[20%] right-4 space-y-2 transition-opacity duration-300"
            style={{ opacity: showControls ? 1 : 0 }}
        >
          {annotations
                ...
                <Tabs defaultValue="all">
                  ...
                  <TabsContent value="all" className="mt-4 space-y-3">
                    {annotations.map(annotation => (
                      <div 
                        key={annotation.id}
                        className="bg-yellow-50 border border-yellow-100 p-3 rounded"
                      >
                  ...
                  <TabsContent value="current" className="mt-4 space-y-3">
                    {annotations.filter(a => a.pagina === pageNumber).length === 0 ? (
                      <p className="text-center text-muted-foreground py-4">
                        Nenhuma anotação na página atual
                      </p>
                    ) : (
                      annotations
                        .filter(a => a.pagina === pageNumber)
                        .map(annotation => (
                          <div 
                            key={annotation.id}
                            className="bg-yellow-50 border border-yellow-100 p-3 rounded"
