...
serve(async (req) => {
  ...
  // Use a chave service_role para realizar escritas no Supabase
  ...
  try {
    ...
    if (hasActiveSub) {
      ...
      // Determinar o nível de assinatura com base no preço
      subscriptionTier = "Premium";
      logStep("Nível de assinatura determinado", { subscriptionTier });
    } else {
      logStep("Nenhuma assinatura ativa encontrada");
    }

    // Atualizar ou criar registro do assinante no banco de dados
    await supabaseClient.from("subscribers").upsert({
      email: user.email,
      user_id: user.id,
